...
## matrix operators source file (supplied separately)
...
mal05.N <- dat.afrD$mal.N + dat.afrE$mal.N + dat.AmA$mal.N + dat.AmB$mal.N + dat.AmD$mal.N + dat.eMedB$mal.N + dat.eMedD$mal.N + dat.EurA$mal.N + dat.EurB$mal.N + dat.EurC$mal.N + dat.SEAB$mal.N + dat.SEAD$mal.N + dat.wPacA$mal.N + dat.wPacB$mal.N
fem05.N <- dat.afrD$fem.N + dat.afrE$fem.N + dat.AmA$fem.N + dat.AmB$fem.N + dat.AmD$fem.N + dat.eMedB$fem.N + dat.eMedD$fem.N + dat.EurA$fem.N + dat.EurB$fem.N + dat.EurC$fem.N + dat.SEAB$fem.N + dat.SEAD$fem.N + dat.wPacA$fem.N + dat.wPacB$fem.N
...
fem.mort.mat <- as.matrix(data.frame(dat.afrD$fem.M,dat.afrE$fem.M,dat.AmA$fem.M,dat.AmB$fem.M,dat.AmD$fem.M,dat.eMedB$fem.M,dat.eMedD$fem.M,dat.EurA$fem.M,dat.EurB$fem.M,dat.EurC$fem.M,dat.SEAB$fem.M,dat.SEAD$fem.M,dat.wPacA$fem.M,dat.wPacB$fem.M))
lab.vec <- c("afrD","afrE","AmA","AmB","AmD","eMedB","eMedD","EurA","EurB","EurC","SEAB","SEAD","wPacA","wPacB")
colnames(fem.mort.mat) <- lab.vec
rownames(fem.mort.mat) <- 0:100

mal.N.mat <- as.matrix(data.frame(dat.afrD$mal.N,dat.afrE$mal.N,dat.AmA$mal.N,dat.AmB$mal.N,dat.AmD$mal.N,dat.eMedB$mal.N,dat.eMedD$mal.N,dat.EurA$mal.N,dat.EurB$mal.N,dat.EurC$mal.N,dat.SEAB$mal.N,dat.SEAD$mal.N,dat.wPacA$mal.N,dat.wPacB$mal.N))
colnames(mal.N.mat) <- lab.vec
rownames(mal.N.mat) <- rownames(mal.mort.mat)
fem.N.mat <- as.matrix(data.frame(dat.afrD$fem.N,dat.afrE$fem.N,dat.AmA$fem.N,dat.AmB$fem.N,dat.AmD$fem.N,dat.eMedB$fem.N,dat.eMedD$fem.N,dat.EurA$fem.N,dat.EurB$fem.N,dat.EurC$fem.N,dat.SEAB$fem.N,dat.SEAD$fem.N,dat.wPacA$fem.N,dat.wPacB$fem.N))
...
plot(yrs,as.vector(colSums(n.matWHO)*totWHO.sex.ratio),type="l",xlab="year",ylab="N",ylim=c(0,1.02*n.maxWHO),xlim=c(yr.now,yr.end))
...
title(main=paste("final N = ",round(fin.popWHO/1e9, 4), " b","; max N = ",round(n.maxWHO/1e9, 4), " b",sep=""),
      sub = paste("delta = ", times.deltaWHO, "x", sep=""))
plot(yrs,as.vector(over65WHO),type="l",xlab="year",ylab="proportion",ylim=c(0,1))
